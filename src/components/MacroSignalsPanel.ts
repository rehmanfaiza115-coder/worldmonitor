...
interface MacroSignalData {
  ...
  signals: {
    ...
    miningDifficulty: { status: string; change30d: number | null };
...
function mapProtoToData(r: GetMacroSignalsResponse): MacroSignalData {
  ...
  return {
    ...
    signals: {
      ...
      miningDifficulty: {
        status: s?.miningDifficulty?.status ?? 'UNKNOWN',
        change30d: s?.miningDifficulty?.change30d ?? null,
      },
      miningCost: {
    ...
    const html = `
      <div class="macro-signals-container">
        ...
        <div class="signals-grid">
          ...
          ${this.renderSignalCard(t('components.macroSignals.signals.miningDifficulty'), s.miningDifficulty.status, formatNum(s.miningDifficulty.change30d), '', '30d change', 'https://mempool.space/mining')}
